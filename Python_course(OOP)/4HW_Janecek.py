...
class Entity:
    ...
    def __init__(self, name: str, coords: tuple[int, int], hit_points: int):
        # "constructor", more precisely "initializer"
        # initialize Entity here
        # name is string such as "Rob"
        # coords is tuple to represent x and y coordinates in the field: (1, 2) means x = 1 and y = 2; expect also negative values
        # hit_points represents count of remaining hit_points as int, 0 or less hit_points means death
    ...
    def take_damage(self, damage_amount: int):
        # method to apply damage on this entity, it takes damage_amount and applies it by own rules to the entity
    ...
    def get_distance(self, other_coords: tuple[int, int]):
        ...
        distance_x = self.coords[0] - other_coords[0]
        distance_y = self.coords[1] - other_coords[1]
        ...
        return absolut(distance_x) + absolut(distance_y)
...
class Rock(Entity):
    ...
    def take_damage(self, damage_amount: int):
        ...
        # each such hit takes him 1 hit_point, smaller hits don't cause any damage to him
        damage_to_hit_rock=10

        if damage_amount > damage_to_hit_rock:
            self.hit_points -= 1

        else:
            print(f"{damage_amount} damage is not enough, you need at least {damage_to_hit_rock} damage")
...
class LivingEntity(Entity):  # tim
    ...
    def take_damage(self, damage_amount: int) -> None:
        # method to apply damage on this entity
        # the whole damage_amount is applied to hit_points
    ...
    def move(self, vector: tuple[int, int]) -> None:
        ...
        coords_loc: tuple[int, int] = (self.coords[0] + vector[0], self.coords[1] + vector[1])
        self.coords = coords_loc
    ...
    def in_range(self, other_entity: Entity) -> bool:
        ...
        if Entity.get_distance(self, other_entity.coords) <= self.attack_range:
            return True
...
class Warrior(LivingEntity):  # willy
    ...
    def take_damage(self, damage_amount: int) -> None:
        # method to apply damage on this entity
        # Warrior is taking damage only when the damage_amount is bigger than his level
        ...
        if damage_amount > self.level:
            ...
            print(f"{damage_amount} damage is not enough, you need at least {self.level} damage to hit Warrior {self.name}")
    ...
    def level_up(self) -> None:
        ...
        LivingEntity.level_up(self)
        self.damage += 1
        self.hit_points += 2


class Archer(LivingEntity):  # rob
